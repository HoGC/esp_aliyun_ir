...
static void ALWAYS_INLINE clr_carrier() {
    ...
    GPIO.out_w1ts |= (0x1 << IR_GPIO_NUM); // equivalent to gpio_set_level(IR_GPIO_NUM, 0);
...
static void IRAM_ATTR ir_tx_timer_handler(ir_encoder_t *encoder) {
    hw_timer_pause();
    int32_t pulse = encoder->get_next_pulse(encoder);
    if (pulse == 0) {
        // Done with transmission
        clr_carrier();
        // gpio_set_level(IR_GPIO_NUM, 1);
