...
static int32_t IRAM_ATTR ir_raw_get_next_pulse(ir_raw_encoder_t *encoder) {
...
int ir_raw_send(int32_t *widths, uint16_t count) {
    ir_raw_encoder_t *encoder = malloc(sizeof(ir_raw_encoder_t) + sizeof(int32_t) * count);
    ...
    memcpy(encoder->data, widths, sizeof(int32_t) * count);
...
static int ir_raw_decode(ir_decoder_t *decoder, int32_t *pulses, uint16_t count,
                         void *decoded_data, uint16_t decoded_size)
{
    if (decoded_size < count * sizeof(int32_t))
        return -1;

    memcpy(decoded_data, pulses, count * sizeof(int32_t));
