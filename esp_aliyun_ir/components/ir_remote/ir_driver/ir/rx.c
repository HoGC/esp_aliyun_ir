...
static void ir_rx_timeout(void *arg) {
    ...
    if (ir_rx_context.buffer_pos && (ir_rx_context.state != ir_rx_state_overflow)) {
        int32_t *pulses = malloc(sizeof(int32_t) * (ir_rx_context.buffer_pos + 1));
        ...
        for (int32_t i = 0, *p = pulses; *p; i++, p++) {
            ir_debug1("%5d ", *p);
            if (i % 16 == 15)
                ir_debug1("\n");
        }
...
void ir_driver_rx_init(uint8_t gpio, uint16_t rx_buffer_size) {
    ir_rx_context.gpio = gpio;
    ir_rx_context.excess = 0;
    ir_rx_context.buffer = malloc(sizeof(int32_t) * rx_buffer_size);
    ir_rx_context.buffer_size = rx_buffer_size;
    ir_rx_context.buffer_pos = 0;

    ir_rx_context.state = ir_rx_state_idle;
    ir_rx_context.timeout = 50000;

    ir_rx_context.timeout_timer = xTimerCreate("ir_rx_timer", 10 / portTICK_PERIOD_MS, pdTRUE, NULL, ir_rx_timeout);
    if (!ir_rx_context.timeout_timer) {
        printf("Failed to create IR rx timer\n");
        return;
    }

    xTimerStart(ir_rx_context.timeout_timer, 0);

    ir_rx_context.receive_queue = xQueueCreate(10, sizeof(int32_t *));
...
void ir_rx_set_excess(int32_t excess) {
