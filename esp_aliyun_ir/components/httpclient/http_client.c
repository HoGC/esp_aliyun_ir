...
static int httpclient_send_header(httpclient_t *client, const char *url, int method, httpclient_data_t *client_data)
{
    ...
    snprintf(buf, sizeof(buf), "%s %s HTTP/1.1\r\nUser-Agent: DoHome-HTTP-Client/2.1\r\nCache-Control: no-cache\r\nConnection: close\r\nHost: %s\r\n", meth, path, host); /* Write request */
...
{
	int ret = 0;
	int timeout_ms = HTTPCLIENT_MAX_RECV_WAIT_MS;
...
HTTPC_RESULT httpclient_recv(httpclient_t *client, httpclient_data_t *client_data)
{
    ...
    int max_len = MIN(HTTPCLIENT_CHUNK_SIZE - 1, client_data->response_buf_len - 1);
    ...
    if (client_data->is_more) {
        ...
        ret = httpclient_recv_data(client, buf, 1, max_len, &reclen);
